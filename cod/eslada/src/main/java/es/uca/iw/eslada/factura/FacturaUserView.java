...
@Route(value = "factura_user", layout = MainLayout.class)
@RolesAllowed("ROLE_USER")

public class FacturaUserView extends VerticalLayout {
    ...
    public FacturaUserView(AuthenticatedUser authenticatedUser, FacturaService facturaService, FacturaPdfGenerator facturaPdfGenerator, ContratoService contratoService){
        ...
        grid.addColumn(new ComponentRenderer<>(HorizontalLayout::new, (layout, factura) -> {
            ...
            Button emailButton = new Button("Enviar", e -> {
                facturaPdfGenerator.sendFacturaEmail(user.get().getEmail(), factura);
            });
        ...
        grid.setItems(facturaService.findFacturasByUserId(user.get().getId()));
